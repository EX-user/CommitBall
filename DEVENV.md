...
- [ ] .NET 8 SDK（CommitBall-Bar、CommitBall-Agent 和 CommitBall-BallShell）: `winget install Microsoft.DotNet.SDK.8`
...
7. Publish CommitBall-Bar: `dotnet publish commitball-bar\commitball-bar.csproj -c Release -r win-x64 --self-contained false -p:PublishSingleFile=true -p:DebugType=None -p:DebugSymbols=false -o commitball-bar\publish`

8. Publish CommitBall-Agent: `dotnet publish commitball-agent\commitball-agent.csproj -c Release -r win-x64 --self-contained false -p:PublishSingleFile=true -p:DebugType=None -p:DebugSymbols=false -o commitball-agent\publish`

9. Publish CommitBall-BallShell: `dotnet publish commitball-ball-shell\commitball-ball-shell.csproj -c Release -r win-x64 --self-contained false -p:PublishSingleFile=true -p:DebugType=None -p:DebugSymbols=false -o commitball-ball-shell\publish`

10. 打包：`cd installer && .\build-installer.ps1`
