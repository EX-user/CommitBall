...
```powershell
Copy-Item weasel\output\weaselx64.dll C:\Windows\System32\weasel.dll -Force
```

### 2. 注册 TSF 文本服务

```powershell
regsvr32 /s weasel\output\weaselx64.dll
```
...
```powershell
Get-ItemProperty "HKLM:\SOFTWARE\Classes\CLSID\{A3F4CDED-B1E9-41EE-9CA6-7B4D0DE6CB0A}\InprocServer32" -ErrorAction SilentlyContinue
```
...
reg add "HKLM\SOFTWARE\WOW6432Node\Rime\Weasel" /v WeaselRoot /t REG_SZ /d "$weaselOutput" /f
reg add "HKLM\SOFTWARE\WOW6432Node\Rime\Weasel" /v ServerExecutable /t REG_SZ /d "WeaselServer.exe" /f
```
...
Windows 设置 → 时间和语言 → 语言和区域 → 中文旁边的 ⋯ → 语言选项 → 添加键盘 → 找到"小狼毫" → 添加。
...
# 2. 取消注册 TSF
regsvr32 /u weasel\output\weaselx64.dll

# 3. 删除 DLL
Remove-Item C:\Windows\System32\weasel.dll -Force

# 4. 清理注册表
Remove-Item -Path "HKLM:\SOFTWARE\WOW6432Node\Rime" -Recurse -Force -ErrorAction SilentlyContinue

# 5. 删除用户数据（必须，否则重装后词典不会重新编译）
...
# 注册表是否正确
Get-ItemProperty "HKLM:\SOFTWARE\WOW6432Node\Rime\Weasel" -ErrorAction SilentlyContinue

# DLL 是否存在
Test-Path C:\Windows\System32\weasel.dll
...
排查步骤：先看 `*.ERROR.*`，再看 `*.WARNING.*`。日志文件名含进程名、时间戳和 PID，例如 `rime.weasel.MECH.user.log.ERROR.20260601-122730.35372.log`。
