...
# 待评阅需求
- [x] 悬浮球录制状态下添加"眨眼"效果
- [x] 悬浮球加上气泡功能，用来向用户发送消息
- [ ] 用c-sharp重构commitball
- [ ] 设计多层级的归档和汇总机制
- [ ] 提供基于sql的agent tool
- [x] db to text 设计过滤机制（已完成：导出 raw / summary / agent 三种视图；兼容 `commitball_*.txt` 默认为 agent 过滤版，旁路保留 `.raw.txt` 与 `.summary.txt`）
- [ ] 代码整体重构
- [x] 提供直接显示到bar/panel上的agent tool；做bar到agent的直接互通（display_panel、show_ball_bubble、Bar 指令模式）

# 高层次需求
- [ ] 反向利用 Agent, 根据最近状况通过bubble与用户沟通
- [ ] 基于多层级归档和检索的agent recall功能, 帮助用户回忆
- [ ] 推测用户状态；帮助用户快速进入工作状态
